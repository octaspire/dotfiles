...
** GNU/Linux
*** EXWM
  Use Emacs as window manager in GNU/Linux if environment variable
  =OCTASPIRE_USE_EMACS_EXWM= is defined to have value =yes= and
  Emacs is not currently running in batch mode.
  #+BEGIN_SRC emacs-lisp
  (when (and (eq system-type 'gnu/linux)
             (string= (getenv "OCTASPIRE_USE_EMACS_EXWM") "yes")
             (not noninteractive))
    (use-package exwm
      :ensure t
      :config
...
** UI
   ...
   (set-face-background hl-line-face "IndianRed4")  ; Change color of highlighted line.
   ...
   ; Maximize frame at startup. This line is from:
